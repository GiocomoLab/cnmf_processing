...
% foldername = 'Z:\2P_data\2AFC_V3\4058541.1\CA1';
foldername = '/home/mplitt/lab_server/2P_data/2AFC_V3/4058541.1/CA1';
        % folder where all the files are located. Currently supported .tif,
        % .hdf5, .raw, .avi, and .mat files
files = subdir(fullfile(foldername,'CA1_001_004_green.h5'));   % list of filenames (will search all subdirectories)
...
if motion_correct
    ...
    h5_files = subdir(fullfile(foldername,'*_nr.h5'));
end
...
%     GUIout = ROI_GUI(data.Y,A(:,keep),P,options,Cn,C(keep,:),b,f,Coor);   
%     options = GUIout{2};
%     keep = GUIout{3};   
    
    plot_components_GUI(data.Y,A(:,keep),C(keep,:),b,f,Cn,options);

%     options.ssub 
%     [Ain,Cin,center] = manually_refine_components(data.Y,A(:,keep),C(keep,:),center,Cn,tau,options);
end
